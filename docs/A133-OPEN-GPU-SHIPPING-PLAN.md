...
- `build/Dockerfile.pf`: add the `gpu-um-src` context and Mesa stage; make the
  ...
  stage gates off the `PF_GPU_REPO` proxy so `a133-open` retains all four
  A133-specific components, without linking `sdl` against the DDK.
...
| B | `think` | `platform`: `platform.lock`, profile schema, `core/profile.py`, `core/pf-build.sh`, tests; `image`: `build/Dockerfile.pf` is-A133 gate re-point (`wpa`, `runtime`, `launcher`, `sdl`) | Lock resolves exact 6.x and Mesa SHAs; separate KM/UM fields and `gpu-um-src` context resolve for a fixture; missing pin/group fails; existing `a133`, `a133-owned`, and `a523` resolved outputs unchanged. **Required, not optional:** the `wpa`, `runtime`, `launcher`, and `sdl` stages are re-pointed off the `PF_GPU_REPO` proxy onto the declarative is-A133 signal (`PF_SOC`/`PF_DEVICE_FAMILY`) — an `a133-open` build must SELECT (not NOT-SHIP) all four stages; B is not done while any of the four still gates on `PF_GPU_REPO` | A |
...
| D | `think` | `image`: `scripts/build-rootfs.sh` or open customizer, `build-sd-image.sh`, SDL/display selection, rootfs tests | Synthetic/open rootfs has discovered 6.x modules + `powervr.ko`, open firmware, Mesa GL/EGL/GLES/Vulkan/GBM/ICD wiring, XR829/runtime services; contains none of the closed PowerVR libraries, `pvrsrvkm.ko`, or `dc_sunxi.ko`; closed A133 fixture remains unchanged | C |
