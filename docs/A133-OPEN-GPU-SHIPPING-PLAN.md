...
- `build/Dockerfile.pf`: add the `gpu-um-src` context and Mesa stage; make the
  ...
  rootfs. Change SDL, WPA, and runtime gates so `a133-open` retains A133-specific
  components without linking SDL against the DDK.
...
| B | `think` | `platform`: `platform.lock`, profile schema, `core/profile.py`, `core/pf-build.sh`, tests | Lock resolves exact 6.x and Mesa SHAs; separate KM/UM fields and `gpu-um-src` context resolve for a fixture; missing pin/group fails; existing `a133`, `a133-owned`, and `a523` resolved outputs unchanged | A |
