...
- `build/Dockerfile.pf`, step B scope: re-point the `wpa`, `runtime`,
  `launcher` (`pf-shell`), and `sdl` stage *gates*, and the `rootfs` stage's
  master `case "$PF_GPU_REPO"` dispatch, off the `PF_GPU_REPO` proxy so
  `a133-open` retains all four A133-specific components (selected, not
  NOT-SHIPPED) and reaches rootfs assembly.
- `build/Dockerfile.pf`, step C scope: add the `gpu-um-src` context and Mesa
  stage; make the DDK `gpu-km` stage closed-profile-only; for the open model
  extract and verify in-tree `powervr.ko` from the kernel stage; build the
  complete Mesa install tree. **Relink the `sdl` stage against that Mesa
  install tree in this same step** — the relink is caused by C (it consumes
  the install tree C builds), so it lands in C, not D; only after this relink
  does `sdl` consume no closed PowerVR UM on the `a133-open` path, which is
  what makes C's "no closed input consumed in open mode" acceptance
  satisfiable without depending on a step (D) sequenced after it.
...
- `boards/tsp/initrd/build-initrd.sh`: it independently hardcodes
  ...
  covered by fixing rootfs alone — **in two places, not one:** the packaged
  `.ko` file, and the init program's `modprobe`/`insmod` module-load
  reference it generates into the initramfs `/init` (loading by module
  *name*, not by file path). Replace both with `powervr.ko` for the open
  model; this is required, since the initrd is what loads the GPU module at
  boot — swapping only the packaged file while the load-by-name reference
  still names the closed module means an otherwise-correct open image fails
  to boot.
...
- `libsdl3-sunxifb`'s DDK linkage is resolved by step C's `sdl` relink above,
  not left open-ended: it must gain a DRM/KMS + Mesa backend for open A133 (or
  a different shipping display/compositor path is selected explicitly); do not
  silently reuse the vendor-linked binary in the "open" image.
...
| C | `think` | `image`: `build/Dockerfile.pf` (Mesa stage build **and the `sdl` stage relink**), Mesa build/stage tests (and `gpu-um-tsp/docker/*` only if its reusable recipe must export a full install tree) | Hermetic source build emits `powervr.ko` from the locked kernel plus a complete Mesa install tree; ICD JSON resolves to an AArch64 library; provenance includes both SHAs. **The `sdl` stage is relinked against the Mesa install tree in this same bead** (caused by C, not deferred to D) — this is what makes "no DDK build or closed input is consumed in open mode" satisfiable within C itself, rather than depending on D's later removal | B |
| D | `think` | `image`: **pure removal across the complete a133 image-assembly path** — `scripts/build-rootfs.sh` (or open customizer), `boards/tsp/initrd/build-initrd.sh`, `scripts/build-sd-image.sh`, rootfs/initrd tests | **Scope is a full sweep of all three assembly scripts, not a per-file list — after C has relinked `sdl` and nothing on the `a133-open` path references them any more, DELETE every closed-GPU-stack artifact left in any of the three scripts:** the eight closed UM `.so` libraries; the closed firmware (`rgx.fw.*`/`rgx.sh.*`); the `pvrsrvkm.ko`/`dc_sunxi.ko` module names in *both* `build-rootfs.sh` and `build-initrd.sh` (the initrd copy is independent of rootfs and boots the device — missing it leaves the open image unable to boot even if rootfs is fixed) — **in `build-initrd.sh` this covers both the packaged `.ko` file AND the init program's runtime module-load reference** (the `modprobe`/`insmod` call by module name the script generates into the initramfs `/init`); swapping only the packaged file while the init still loads by the closed module's name means the open GPU module never loads at boot; and the hard-coded `/lib/modules/4.9.191` kernel-version path wherever it appears. Replace with the open equivalents: `powervr.ko` — as both the packaged file and the load-by-name target — in both rootfs and initrd; the open firmware `powervr/rogue_22.102.54.38_v1.fw` (from L4's `pvr-fw-open-22.102.54.38` group); the discovered 6.x kernel-modules path. `build-sd-image.sh`'s unconditional `pvrsrvkm.ko`-exists assertion is replaced by a profile/model-specific GPU artifact check. **Acceptance is grep-verifiable and closes the whole class:** a grep across all three scripts' `a133-open` code path for the closed-stack literals above — including both the `.ko`-suffixed filenames and the bare module names (`pvrsrvkm`, `dc_sunxi`) that a `modprobe`/`insmod` call uses without the extension — plus `4.9.191`, the eight closed UM library names, `rgx.fw`, `rgx.sh`, returns no match; the `a133-open` image assembles and boots with only the open module and firmware present, loaded by name; the closed `a133` fixture and its assembly output remain unchanged | C |
