...
grep -Fx 'ARG PF_VARIANT=dev' "$dockerfile" >/dev/null
...
grep -Fx 'FROM hwprobe-${PF_VARIANT} AS hwprobe' "$dockerfile" >/dev/null
! grep -q 'PF_HWPROBE_STAGE' "$dockerfile"
grep -Eq '^COPY --from=hwprobe[[:space:]]+/out[[:space:]]+/work/hwprobe$' "$dockerfile"

release_body="$(sed -n '/^FROM ${PF_CONTAINER} AS hwprobe-release$/,/^FROM hwprobe-${PF_VARIANT} AS hwprobe$/p' "$dockerfile")"
